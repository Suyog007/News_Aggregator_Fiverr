...
mycol = mydb['api']
res = requests.get('http://news.acotonou.com/',headers=header)
...
for url in link:
    try:
        ...
        soup = BeautifulSoup(res.text,'html.parser')
        title = soup.find_all('h1',{'class':'FontArticleMainTitle'})
        for x in title:
            if x is not None:
                dict1['title'] = x.text
        img = soup.find_all('img',{'class':'imgArt'})
        for x in img:
            if x is not None:
                    dict1['image'] = x.get('src')
        dict1['url'] = url
        dict1['country'] = 'Benin'
        dict1['language'] = 'fr'
        if dict1['title'] == '':
