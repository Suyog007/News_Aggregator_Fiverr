...
mycol = mydb['api']
res = requests.get('https://www.lnr-dz.com/',headers=header)
...
for url in link:
    try:
        ...
        dict1['title'] = ''
        dict1['image'] = ''
        if not url.startswith('https://www.lnr-dz.com/'):
            url = 'https://www.lnr-dz.com/'+url
        res = requests.get(url,headers=header)
        soup = BeautifulSoup(res.text,'html.parser')
        title = soup.find_all('article',{'class':'post-content'})
        for x in title:
            if x is not None:
                head= x.find('h2')
                if head is not None:
